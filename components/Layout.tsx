...
const Layout: React.FC<LayoutProps> = ({ children, stats, username }) => {
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-2xl border-b border-slate-200/60 transition-all">
        <div className="max-w-[1600px] mx-auto px-6 h-16 md:h-28 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 md:gap-8">
            <div className="flex items-center gap-2 md:gap-4 px-3 py-2 md:px-6 md:py-4 bg-orange-50/50 rounded-2xl border border-orange-100/50 group transition-all hover:bg-orange-50">
              <Flame className="w-4 h-4 md:w-7 md:h-7 text-orange-500 fill-orange-500 group-hover:animate-bounce" />
              <div className="flex flex-col">
                <span className="font-extrabold text-slate-900 text-sm md:text-2xl leading-none">{stats.streak}</span>
                <span className="hidden md:block text-[8px] font-black text-orange-400 uppercase tracking-widest mt-1">Стрик</span>
              </div>
            </div>

            <div className="flex items-center gap-2 md:gap-4 px-3 py-2 md:px-6 md:py-4 bg-amber-50/50 rounded-2xl border border-amber-100/50 group transition-all hover:bg-amber-50">
              <Trophy className="w-4 h-4 md:w-7 md:h-7 text-amber-500 fill-amber-500 group-hover:rotate-12 transition-transform" />
              <div className="flex flex-col">
                <span className="font-extrabold text-slate-900 text-sm md:text-2xl leading-none">{stats.points}</span>
                <span className="hidden md:block text-[8px] font-black text-amber-500 uppercase tracking-widest mt-1">IQ Очки</span>
              </div>
            </div>

            <div className="flex items-center gap-4 md:gap-6 pl-4 md:pl-10 border-l border-slate-200/60">
