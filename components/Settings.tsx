    ...
    <MotionDiv initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto space-y-8">
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
        <div className="flex items-center gap-4 mb-8">
          <div className="bg-orange-50 p-3 rounded-2xl"><User className="w-6 h-6 text-orange-600" /></div>
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <div className="bg-sky-50 p-3 rounded-2xl"><ListFilter className="w-6 h-6 text-sky-600" /></div>
            <h3 className="font-black text-slate-800 text-lg uppercase tracking-tight">Ваши интересы (макс. 5)</h3>
          </div>
          <button onClick={loadCategories} className="p-2 text-slate-400 hover:text-orange-600 transition-colors"><RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} /></button>
        </div>
          ...
          {categories.map(cat => {
            ...
            return (
              <MotionButton key={cat.name} whileHover={{ scale: 1.05 }} onClick={() => toggleCategory(cat.name)}
                ...
                `}>
                <span className={`font-black text-[10px] uppercase tracking-wider ${isSelected ? 'text-orange-700' : 'text-slate-500'}`}>{cat.name}</span>
                ...
                {isSelected && <div className="absolute top-1 right-1 bg-orange-500 rounded-full p-1 border-2 border-white"><CheckCircle2 className="w-2 h-2 text-white" /></div>}
              </MotionButton>
            );
          })}
        </div>
